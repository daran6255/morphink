...
// Interactive Testimonial Mockup Overlay within Visual Frame
const InteractiveTestimonialPreview = styled(Box)(({ theme }) => ({
  position: 'absolute',
...
const TRUST_CREDENTIALS = [
  {
    ...
    detail: 'Quotes rooted in real deliverables — from submission-ready DEPwD reports to custom MIS platforms.',
  },
  {
    icon: HandshakeOutlinedIcon,
    tone: 'accent' as const,
    title: 'Long-Term Partnerships',
    detail: 'Multi-year relationships spanning healthcare tech, social foundations, and higher education.',
  },
  {
    icon: LightbulbOutlinedIcon,
    tone: 'info' as const,
    title: 'Direct Partner Reflections',
    detail: 'Client input that directly fuels our continuous curriculum updates and internal tooling evolution.',
  },
]
      ...
      <Grid container spacing={{ xs: 5, md: 6, lg: 8 }} sx={{ alignItems: 'center' }}>
        ...
        <Grid size={{ xs: 12, md: 7, lg: 6 }}>
          <Stack spacing={3.5}>
            <Chip
              icon={<PulseDot />}
              label={testimonialsHeroData.eyebrow}
              variant="outlined"
              size="medium"
              sx={(theme) => ({
            ...
            >
              {testimonialsHeroData.headline} <GradientText>{testimonialsHeroData.headlineHighlight}</GradientText>
            </Typography>
            ...
            >
              {testimonialsHeroData.subheadline}
            </Typography>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ pt: 1 }}>
              <Button
                tone="primary"
                variant="contained"
                href={testimonialsHeroData.primaryCta.href}
                size="large"
              ...
              >
                {testimonialsHeroData.primaryCta.label}
              </Button>

              <Button
                variant="outlined"
                size="large"
                href={testimonialsHeroData.secondaryCta.href}
                sx={(theme) => ({
                  px: 3.5,
                  py: 1.6,
                  fontSize: '1.0625rem',
              ...
              >
                {testimonialsHeroData.secondaryCta.label}
              </Button>
            </Stack>

            {/* Key Stat Pills */}
            <Stack direction="row" spacing={1.5} sx={{ pt: 1, flexWrap: 'wrap', gap: 1.5 }}>
              {testimonialsHeroData.stats.map((stat, idx) => (
                ...
                <InteractiveTestimonialPreview>
                  <Stack direction="row" spacing={1} sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
                    <Stack direction="row" spacing={0.75} sx={{ alignItems: 'center' }}>
                      <StarRateRoundedIcon sx={{ fontSize: 20, color: 'accent.main' }} />
                      <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.825rem' }}>
                        Partner Impact Endorsements
                      </Typography>
                    </Stack>
                    <Box
                      sx={(theme) => ({
                  ...
                  <Typography variant="caption" sx={{ fontStyle: 'italic', color: 'text.secondary', lineHeight: 1.4, fontSize: '0.78125rem' }}>
                    "Having both the exact technical code fix and an alternative UX recommendation meant we resolved every gap smoothly."
                  </Typography>

                  <Typography variant="caption" sx={{ fontWeight: 700, color: 'primary.main', fontSize: '0.75rem' }}>
                    — Lead Product Manager, Fintech Platform
                  </Typography>
                </InteractiveTestimonialPreview>
              </VisualImage>
            </VisualFrame>
